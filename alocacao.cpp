...
aluno *insere_aluno(aluno *v, int *n, aluno novo) {
  aluno *vetnovo = new (nothrow)aluno[*n+1];

  if(!vetnovo){
    cerr << "Erro na alocacao" << endl;
    return 0;
  }
...
Pseudocódigo:
   para i de 0 até o tamanho do vetor menos 1
      se a matrícula do i-ésimo elemento do vetor é igual à 'matrícula' passada
como parâmetro copie último elemento do vetor para a posição i realoque vetor
para conter um elemento a menos, pondo endereço em v se não for possível
realocar, imprima mensagem de erro e retorne reduza em 1 o tamanho do vetor
(decrementar *n) break retorne o endereço de memória do vetor realocado
*/
aluno *remove_aluno(aluno *v, int *n, int matricula) {
  for (int i = 0; i < *n; i++) {
    if (v[i].matricula == matricula) {
      v[i] = v[*n-1];
      aluno *novovetor = new aluno[*n - 1];
      if (novovetor == NULL) {
        cerr << "Erro na relocacao de memoria." << endl;
        return v;
      }
      for (int j = 0; j < *n - 1; j++) {
        novovetor[j] = v[j];
